...
import {
  errorResponseSchema,
  trainScheduleWithTimestampsSchema,
  createTrainScheduleBodySchema,
  trainSchedulesListResponseSchema,
...
export default async function trainScheduleRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (
        routeStationIds.length !== providedStationIds.length ||
        !routeStationIds.every((id) => providedStationIds.includes(id))
      ) {
        return reply
          .code(400)
          .send({ error: 'Station times must be provided for all stations in the route' });
      }
      ...
      try {
        ...
        for (let i = 0; i < stationTimes.length; i++) {
          ...
          const routeStation = train.trainRoute.routeStations.find(
            (rs) => rs.currentStationId === stationTime.stationId,
          );

          if (!routeStation) {
            return reply
              .code(400)
              .send({ error: `Station ${stationTime.stationId} not found in route` });
          }
        ...
        if (error.code === 'P2002') {
          return reply
            .code(409)
            .send({ error: 'Schedule already exists for this train on this date' });
        }
        throw error;
      }
    },
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const filteredRoutes = validRoutes.filter((route) => {
        const fromStation = route.routeStations.find(
          (rs) => rs.currentStationId === fromStationIdNum,
        );
  ...
  fastify.get(
    '/train-schedules/:id/available-seats',
    {
      preHandler: (fastify as any).authenticate,
      schema: {
    ...
    async (request, reply) => {
      const { id } = request.params as { id: string };
      const { fromStationId, toStationId } = request.query as {
      ...
      const fromStationInRoute = trainSchedule.train.trainRoute.routeStations.find(
        (rs) => rs.currentStationId === fromStationId,
      );
      const toStationInRoute = trainSchedule.train.trainRoute.routeStations.find(
